...
/// The main function: take a config file, and run the entire processing chain:
///
...
pub async fn run(config_fname: &str) {
    // Load config.
    ...
    let seen_guids: HashSet<Guid> =
        utils::load_json(&config.db_file).await.unwrap_or_default();
